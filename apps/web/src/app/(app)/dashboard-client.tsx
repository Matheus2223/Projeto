...
import type { Trend, ContentIdea, PostItem, BankItem, NewsItem, DailyReport } from "@/lib/data/types";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        ...
        <StatCard index={1} label="Ideias geradas hoje" value={String(ideas.length)} delta={8.1} icon={Lightbulb} iconClassName="bg-primary/15 text-primary" />
        <StatCard index={2} label="Score de oportunidade" value={`${dailyReport.opportunityScore}/100`} delta={5.6} icon={Target} iconClassName="bg-success/15 text-success" />
        <StatCard index={3} label="Probabilidade de viralização" value={`${dailyReport.viralProbability}%`} delta={-2.3} icon={Zap} iconClassName="bg-info/15 text-info" />
      </div>
