...
export function ConcorrenciaClient({ initialCompetitors }: { initialCompetitors: Competitor[] }) {
  ...
  const addCompetitor = async () => {
    ...
    const platforms = rng.pickMany(["instagram", "tiktok", "facebook"] as const, 2);
    const finalHandle = handle.trim() || `@${name.toLowerCase().replace(/\s+/g, "")}`;

    const created = await createCompetitorAction({ name: name.trim(), handle: finalHandle, platforms, segment });

    const newCompetitor: Competitor = created ?? {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={addCompetitor} disabled={!name.trim() || saving}>{saving ? "Salvando..." : "Adicionar"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
